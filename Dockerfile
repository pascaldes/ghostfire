...
# install Ghost / optional: --verbose
    su-exec node ghost install "$GHOST_VERSION" --db sqlite3    \
        --no-prompt --no-stack --no-setup --dir "$GHOST_INSTALL"; \
    \
# tell Ghost to listen on all ips and not prompt for additional configuration
    cd "$GHOST_INSTALL"; \
    su-exec node ghost config --ip 0.0.0.0 --port 2368          \
        --no-prompt --db sqlite3 --url http://localhost:2368    \
